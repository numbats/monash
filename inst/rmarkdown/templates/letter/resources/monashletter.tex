...
\makeatletter
\def\strlen#1{\strlenimpl0#1\relax}
\def\strlenimpl#1#2{%
\ifx\relax#2 \the\numexpr#1\relax
\else
\expandafter\strlenimpl\expandafter{%
...
$if(fromemail)$\faicon{envelope} $fromemail$\quad $endif$
$if(fromphone)$\faicon{phone} $fromphone$\quad $endif$
$if(fromWWW)$ \ifnum\strlen{$fromWWW$}>20 \\[0.1cm]\fi \faicon{home} $fromWWW$ $endif$\\[0.1cm]
